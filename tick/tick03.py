...
lines = cv2.HoughLinesP(dilate, 1, np.pi/180, 31, 70, 25)
...
for x1, y1, x2, y2 in line:
	if x1 == x2:
		...
		k = (y2 - y1)/(x2 - x1)
		if k < 0:#在左侧
			if abs(k) < tan(25*np.pi/180) and abs(k) > tan(23*np.pi/180):
				filter_line.append([x1, y1, x2, y2])
		else:
			if abs(k) > tan(30*np.pi/180) and abs(k) < tan(35*np.pi/180):
				filter_line.append([x1, y1, x2, y2])
